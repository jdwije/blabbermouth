...
export default class Distributor implements Blabbermouth.IDistributor {
    ...
    publish(topicId, event, subscriber) {
        ...
        Promise.all(
            this.topics[topicId]
                .subscribers
                .map((sub) => sub(event, bm))
        ).then((data) => {
            subscriber && subscriber(eventFactory(topicId, data.map(() => {
                return {
                    uuid: event.uuid,
                    topicId,
                };
    ...
    async request(topicId, event, subscriber?) {
        ...
        while (i < subscribers.length) {
            data.push(await subscribers[i](event, bm));
            i++;
        }

        subscriber && subscriber(eventFactory(topicId, data), bm);
