  ...
  [{
    ...
    exec: async () => {
      ...
      bm.createTopic(topics.httpRequest)
        ...
        .registerHandler([topics.writeFileRequest.id], services.writeFile(bm))
        .registerHandler(
        [topics.writeFileResponse.id],
        services.readFile(bm)
        )
        .registerHandler([topics.readFileResponse.id], async (topicId, event) => {
      ...
      while (i < maxMessages) {
        i++;
        await bm.publish(topics.writeFileRequest.id, {
